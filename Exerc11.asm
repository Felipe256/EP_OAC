...
.globl main
main:
	li $a3, 5 		#variavel para parada do loop, min: 1 max: infinito
	subu $sp, $sp, 20	#coloca 5 espacos na pilha
	li $a0, 2 		#valor de G2
	sw $a0, 16($sp)		#coloca no 5º espaco, o valor de G2
	li $a0, 1 		#valor de F2
	sw $a0, 12($sp) 	#coloca no 4º espaco, o valor de F2
	li $a0, 1 		#valor de G1
	sw $a0, 8($sp) 		#coloca no 3º espaco, o valor de G1
	li $a0, 2 		#valor de F1
	sw $a0, 4($sp) 		#coloca no 2º espaco, o valor de F1 
	li $a0, 1 		#valor de i
	sw $a0, 0($sp) 		#coloca no 1º espaco, o valor de i 
	#jal imprimeValor
	jal loop
	jal imprimeValorFinal
	li $v0, 10
	syscall 
loop:
	#lw $a0, 0($sp)
...
imprimeValor:
	...
	jr $ra
imprimeValorFinal:
	lw $a1, 0($sp) 		#recebe o valor de i + 1
	subu $a1, $a1, 1
	la $a0, letraF 		#colocando a letra F para imprimir
	li $v0, 4 		#codigo pra imprimir caracteres
	syscall
